    ...
    const columns = [
        {
            ...
            render: (value: string, product: Product) => (
                <div className="flex items-center justify-end gap-4 lg:justify-normal">
                    <div>
                        <p className="text-muted-foreground group-hover:text-primary font-semibold transition-colors">{value}</p>
        ...
        },
        {
            ...
            render: (value: string) => {
                ...
                return (
                    <Badge
                        className={`bg-gradient-to-r ${typeColors[value]} rounded-full px-3 py-1 font-mono text-xs tracking-wider text-white uppercase shadow-lg transition-transform duration-200 hover:scale-105`}
                    >
                        {value.replace('_', ' ')}
                    </Badge>
                );
        ...
        },
        {
            ...
            label: 'Price',
            sortable: true,
            render: (value: number) => <span className="font-mono text-sm font-semibold text-green-400">Rp {value.toLocaleString('id-ID')}</span>,
        },
        {
